import type { InFlightSnapshot, InFlightTracker } from "./in-flight";
...
export class CheckpointStore<T> {
  ...
  constructor(tracker?: InFlightTracker) {
  ...
  save(state: T, label?: string): CheckpointId {
    ...
    const inFlight = this.tracker?.snapshot();
    this.checkpoints.push({ id, state: snapshot, timestamp: Date.now(), label, inFlight });
    return id;
  }
  ...
  rollbackTo(id: CheckpointId): T {
    ...
    // Cancel any operations that were in-flight when this checkpoint was taken
    if (this.tracker) this.tracker.cancelAll();
